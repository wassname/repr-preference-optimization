...
class ETHERLinear(ETHERLayer):
    ...
    def reset_parameters(self):
        ...
        if hasattr(self, "ether_R3"):
            ...
            if hasattr(self, "ether_R4"):
                self.ether_R4.data = -torch.empty_like(self.ether_R3).copy_(
                    self.ether_R3
                )
    ...
    def forward(self, x: torch.Tensor):
        ...
        ether = nn.functional.linear(
            input=self.ether_dropout(x), weight=filt, bias=bias_term
        )
        return ether
    ...
    def etherplus(self, R1, R2):
        ...
        H = (
            I
            - torch.bmm(R1.transpose(1, 2), R1) / torch.bmm(R1, R1.transpose(1, 2))
            + torch.bmm(R2.transpose(1, 2), R2) / torch.bmm(R2, R2.transpose(1, 2))
        )
        return H
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.linear_up = torch.nn.Linear(
            out_features // reduction, out_features, bias=bias, **kwargs
        )
